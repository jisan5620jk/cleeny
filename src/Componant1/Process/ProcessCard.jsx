...
const ProcessCard = ({processIcon,processTitle,boxNumber,processDesc}) => {
    return (
      <div className="text-center group">
        <div className="w-28 h-28 bg-white rounded-2xl flex items-center justify-center shadow-shadow m-auto relative z-10 before:absolute before:top-0 before:left-0 before:w-full before:scale-0 before:h-full before:rounded-2xl before:bg-SecondaryColor-0 before:transition-all before:duration-500 group-hover:before:scale-100">
          ...
          />
          <h6 className="h-[26px] w-[26px] rounded-full bg-PrimaryColor-0 text-sm flex justify-center items-center text-HeadingColor-0 font-Inter absolute top-1/2 -translate-y-1/2 -right-[13px]">{boxNumber}</h6>
        </div>
        ...
        <p className="font-Poppins text-TextColor-0">{processDesc}</p>
      </div>
    );
};
